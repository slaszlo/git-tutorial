...
mkdir -p example-railway-repo
pushd example-railway-repo
...
git checkout Rammstein-Du_hast; git merge --no-edit  --no-ff master
...
git checkout Aqua-Baribie_Girl;  git merge master --no-edit
